...
#' 
#' @details When \code{format = "alphabeta"}, p-values are formatted:
#' \enumerate{
...
pvalString <- function(p, format=c("alphabeta", "exact", "scientific"),
                       digits=3, ...){
  format <- match.arg(format, c("alphabeta", "exact", "scientific"))
  ...
  if (format == "alphabeta"){
    ps <- ifelse(p > .99, "> 0.99",
                 ifelse(p > 0.10, format(round(p, 2), digits=2),
